/**
 * Imports the first N seconds of the Freesound heat MP3 into heat.wav.
 * Source: assets/sounds/heat_source.mp3 (or HEAT_SOUND_SOURCE env path).
 */
...
const DEFAULT_SOURCE = resolve(root, "assets/sounds/heat_source.mp3");
const OUT_WAV = resolve(root, "assets/sounds/heat.wav");
const CLIP_SECONDS = 5;
const TARGET_RATE = 44100;
const TARGET_PEAK = 0.88;

function resolveSourcePath() {
  const env = process.env.HEAT_SOUND_SOURCE;
...
async function main() {
  ...
  if (!existsSync(sourcePath)) {
    console.error(`[import-heat-sound] missing source: ${sourcePath}`);
    process.exit(1);
  }

  if (sourcePath !== DEFAULT_SOURCE) {
    copyFileSync(sourcePath, DEFAULT_SOURCE);
    console.log(`[import-heat-sound] copied source to ${DEFAULT_SOURCE}`);
  }
  ...
  console.log(`[import-heat-sound] wrote ${OUT_WAV} (${durationSec}s from ${sourcePath})`);
}

main().catch((error) => {
  console.error("[import-heat-sound] failed:", error);
  process.exit(1);
});
